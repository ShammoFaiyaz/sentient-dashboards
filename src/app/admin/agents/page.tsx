...
export default function AdminAgents() {
  ...
  const slug = useMemo(() => {
    const key = (Object.keys(NICHES) as Array<keyof typeof NICHES>).find(s => pathname.startsWith("/" + s));
    return key ?? null;
  }, [pathname]);

  const config = slug ? NICHES[slug] : null;
  const allowedIds = config?.agentIds ?? null;
  const roleLabels = config?.roles ?? ["Student", "Teacher", "Admin"];

  const sections = roleLabels.map((label) => {
    const suRole =
      slug ? nicheRoleToSuRole(slug, label)
      : label.toLowerCase().includes("admin") ? "admin"
      : label.toLowerCase().includes("teacher") || label.toLowerCase().includes("manager") ? "teacher"
      : "student";

    const items = (allowedIds
      ? agentsByRole(suRole).filter(a => allowedIds.includes(a.id))
      : agentsByRole(suRole));
  ...
  return (
    <div className="mx-auto max-w-7xl px-2 py-6">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="text-sm">Role
              <select value={role} onChange={(e) => setRole(e.target.value as any)} className="mt-1 w-full rounded-md border border-line/60 p-2 text-sm">
                ...
                <option value="teacher">Practitioner</option>
                <option value="student">Customer/Patient/Guest</option>
              </select>
            </label>
            ...
            <div className="sm:col-span-2">
              ...
              <div className="mt-1 grid gap-2 sm:grid-cols-2">
                <input value={bullet1} onChange={(e) => setBullet1(e.target.value)} className="w-full rounded-md border border-line/60 p-2 text-sm" placeholder="e.g., Segment & personalize journeys" />
                <input value={bullet2} onChange={(e) => setBullet2(e.target.value)} className="w-full rounded-md border border-line/60 p-2 text-sm" placeholder="e.g., A/B optimize content" />
              </div>
            </div>
          </div>
      ...
      {sections.map(({ label, items }) => (
        ...
        >
          <AgentsGrid items={items} onToggle={toggleOnline} onRemove={removeAgent} />
        </Section>
      ))}
    </div>
  );
