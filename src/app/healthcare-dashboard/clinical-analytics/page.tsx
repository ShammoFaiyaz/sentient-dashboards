...
export default function ClinicalAnalyticsPage() {
  const role = useNicheRole("healthcare-dashboard", "Analyst");
  return (
    <div className="mx-auto max-w-7xl px-2 py-6">
      <h1 className="text-2xl font-semibold text-primary">Clinical Analytics</h1>
      <div className="text-sm text-muted mb-3">Role: {role}</div>
      <div className="grid gap-4 md:grid-cols-2">
        <Panel title="Readmission Rate" />
        <Panel title="Length of Stay Trend" />
        <Panel title="Procedure Outcomes" />
        <Panel title="Population Health Segments" />
      </div>
    </div>
  );
}
